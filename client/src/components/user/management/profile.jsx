...
const Profile = () => {
  ...
  const handleEdit = (item) => {
    ...
    // parse contactInfo into email and phone
    const ci = item.contactInfo || '';
    if (ci.includes('|')) {
      const parts = ci.split('|').map(p => p.trim());
      setEditContactEmail(parts[0] || '');
      setEditContactNumber(parts[1] || '');
    } else {
      setEditContactEmail(ci || '');
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      // validate contact email and optional phone
      const emailPattern = /^\S+@\S+\.\S+$/;
      const phonePattern = /^[+]?\d[\d\s().-]{5,}$/;
      if (!editContactEmail || !emailPattern.test(editContactEmail)) {
        setError('A valid email address is required');
        setLoading(false);
        return;
      }
      if (editContactNumber && !phonePattern.test(editContactNumber)) {
        setError('If provided, phone number must be valid');
        setLoading(false);
        return;
      }

      const combinedContact = editContactNumber ? `${editContactEmail} | ${editContactNumber}` : editContactEmail;
