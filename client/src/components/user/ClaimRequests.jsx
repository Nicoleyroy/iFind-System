...
export default function ClaimRequests() {
  ...
  const calculateMatchScore = (item, claim) => {
    ...
    // Description similarity (70 points) - MOST IMPORTANT
    // Compares the found item description with the claimant's proof description
    maxScore += 70;
    const foundDesc = (foundItem.description || '').toLowerCase();
    const claimDesc = (claim.proofOfOwnership || '').toLowerCase();
    
    if (!claimDesc || claimDesc.length < 10) {
      matchDetails.push({ label: 'Missing ownership description', status: 'no-match' });
    } else if (!foundDesc || foundDesc.length < 10) {
      // If found item has no description, give partial credit if they provided one
      score += 35;
      matchDetails.push({ label: 'Description provided', status: 'weak-match' });
    } else {
      // Compare descriptions word by word
      const descWords = foundDesc.split(/\s+/).filter(w => w.length > 3);
      ...
      if (descSimilarity > 0.5) {
        score += 70;
        matchDetails.push({ label: 'Description closely matches', status: 'match' });
      } else if (descSimilarity > 0.2) {
        score += 35;
        matchDetails.push({ label: 'Partial description match', status: 'weak-match' });
      } else {
        matchDetails.push({ label: 'Description does not match', status: 'no-match' });
      }
    }

    // Proof image provided (30 points)
    // Having visual evidence significantly increases confidence
    maxScore += 30;
    if (claim.imageUrl) {
      score += 30;
      matchDetails.push({ label: 'Proof image provided', status: 'match' });
    } else {
      matchDetails.push({ label: 'No proof image uploaded', status: 'no-match' });
    }
                      ...
                      <div className="p-4 sm:p-6">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
                          <div className="flex gap-4 flex-1">
                            {/* Item image and Proof image */}
                            <div className="flex gap-2">
                              <div className="w-20 h-20 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                                {(() => {
                                  const itemImage = claim.itemId?.images && claim.itemId.images.length > 0 
                              ...
                              {claim.imageUrl && (
                                <div className="w-20 h-20 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0 border-2 border-blue-200">
                                  <img
                                    src={claim.imageUrl}
                        ...
                        <div className="bg-white rounded-lg p-4 mb-4 border border-gray-200">
                          <h4 className="text-sm font-semibold text-gray-900 mb-3">Match Summary</h4>
                          <div className="flex flex-wrap gap-4">
                            {matchData.details.slice(0, 3).map((detail, idx) => (
                              <div key={idx} className="flex items-center gap-2 text-sm">
                                ...
                                <span className={detail.status === 'match' ? 'text-green-700 font-medium' : detail.status === 'weak-match' ? 'text-yellow-700 font-medium' : 'text-gray-600'}>
                                  {detail.label}
                                </span>
                              </div>
                            ))}
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    {(() => {
                      const itemImage = selectedClaim.itemId?.images && selectedClaim.itemId.images.length > 0 
                        ? selectedClaim.itemId.images[0] 
                        : selectedClaim.itemId?.imageUrl;
                      ...
                      return itemImage ? (
                        <img
                          ...
                          onClick={() => setPreviewUrl(itemImage)}
                        />
                      ) : (
