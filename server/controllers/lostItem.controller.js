...
const createLostItem = async (req, res) => {
  try {
    ...
    if (!normalizedContact || (!emailPattern.test(normalizedContact) && !phonePattern.test(normalizedContact))) {
      return res.status(400).json({ message: 'Enter valid contact information.' });
    }
...
const updateLostItem = async (req, res) => {
  try {
    const { id } = req.params;
    const { name, category, location, date, contactInfo, description, imageUrl, images, status, moderatorId } = req.body || {};
    
    const oldItem = await LostItemModel.findById(id);
    if (!oldItem) {
      return res.status(404).json({ message: 'Lost item not found' });
    }
    ...
    if (status !== undefined) {
      ...
      if (validStatuses.includes(status)) {
        if (status === 'Returned') {
          ...
          updateData.status = status;
          if (oldItem.status === 'Returned' || oldItem.wasReturned) {
            updateData.wasReturned = true;
          }
        } else {
          updateData.status = status;
    ...
    // Audit logging for status change or update by moderator
    if (moderatorId && (status !== undefined && oldItem.status !== status)) {
      await AuditLogModel.create({
        moderatorId,
        action: status === 'Deleted' ? 'item_deleted' : 'item_verified',
        targetType: 'LostItem',
        targetId: item._id,
        details: `${status === 'Deleted' ? 'Deleted' : 'Updated status of'} lost item "${item.name}" to ${status}`,
        metadata: {
          itemName: item.name,
      ...
      await AuditLogModel.create({
        ...
        targetType: 'LostItem',
        targetId: item._id,
        details: `Updated lost item "${item.name}"`,
  ...
  try {
    ...
    if (moderatorId) {
      await AuditLogModel.create({
        ...
        targetType: 'LostItem',
        targetId: id,
        details: `Permanently deleted lost item "${item.name}"`,
        metadata: { itemName: item.name }
      });
    }
  ...
  try {
    ...
    if (moderatorId) {
      await AuditLogModel.create({
        ...
        targetType: 'LostItem',
        targetId: item._id,
        details: `Marked lost item "${item.name}" as deleted`,
        metadata: { itemName: item.name }
      });
    }
...
const exportLostItemsPdf = async (req, res) => {
  try {
    ...
    const addHeaderFooter = () => {
      ...
      // Simple centered header for all pages
      doc.save();
      ...
      doc.text(h2, (pageWidth - h2w) / 2, 28, { lineBreak: false });
      doc.fontSize(9);
      const h3 = 'Official Report';
      const h3w = doc.widthOfString(h3);
      doc.text(h3, (pageWidth - h3w) / 2, 42, { lineBreak: false });
      doc.restore();
