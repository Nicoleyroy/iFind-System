...
const updateFoundItem = async (req, res) => {
  try {
    ...
    const oldItem = await FoundItemModel.findById(id);
    if (!oldItem) {
      return res.status(404).json({ message: 'Found item not found' });
    }
    ...
    if (moderatorId && (status !== undefined && oldItem.status !== status)) {
      await AuditLogModel.create({
        ...
        targetId: item._id,
        details: `${status === 'Deleted' ? 'Deleted' : 'Updated status of'} found item "${item.name}" to ${status}`,
        metadata: {
          itemName: item.name,
      ...
      await AuditLogModel.create({
        ...
        targetType: 'FoundItem',
        targetId: item._id,
        details: `Updated found item "${item.name}"`,
  ...
  try {
    ...
    if (moderatorId) {
      await AuditLogModel.create({
        ...
        targetType: 'FoundItem',
        targetId: id,
        details: `Permanently deleted found item "${item.name}"`,
        metadata: { itemName: item.name }
      });
    }
  ...
  try {
    ...
    if (moderatorId) {
      await AuditLogModel.create({
        ...
        targetType: 'FoundItem',
        targetId: item._id,
        details: `Marked found item "${item.name}" as deleted`,
        metadata: { itemName: item.name }
      });
    }
